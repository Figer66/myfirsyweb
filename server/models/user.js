...
module.exports = {
  ...
  login: function (username, password, cb) {
      ...
      console.log('登录里面的username是：'+username);
      console.log('登录里面的password是：'+password);
    pool.getConnection(function (err, connection) {
      if (err) throw err;
      connection.query('SELECT id,user_name,user_account,right_id,user_mobile FROM `users` WHERE `user_name`=? AND `user_psd`=?', [username, password], function (err, result) {
        if (err) throw err;
        console.log('登录里面的resultA是：')
